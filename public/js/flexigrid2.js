...
(function($){
	...
	$.addFlex2 = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 autoload: true,
			 blockOpacity: 0.5,
			sortorder: 'asc', // mikeB default sortorder
			 onDragCol: false,
			 onToggleCol: false,
			 onChangeSort: false,
			 onSuccess: false,
...
// ROW CLASS NAME ASSIGNMENT
							tr.className = 'flexrow';
							if(row.cell[10] > 0)
								{
								tr.className += ' sub cid' + row.cell[9];
								}
							else
								{
								tr.className += ' parent id' + row.cell[9];
								}
			...
			changeSort: function(th) { //change sortorder
				...
				if (p.onChangeSort)
					...
					this.inPlaceSort(); // mikeB
				else // mikeB
					this.populate();				
			
			},

			inPlaceSort: function() { // entire function added mikeB
			    ...
			    if (isNaN(col) || col < 0) {
					...
					confirmRedirect("Sorting is somehow not configured properly - couldn't find the header for name '"+sortname+"'" , "noRedirect");
			    }
...
// THIS IS WHERE A ROW CLICK IS PROCESSED

										if($(this).hasClass("parent"))
											{
											var id = String($(this).attr('class')).split(" ");
											var el;
											for(el in id)
												{
												if(id[el].indexOf("id") > -1)
													{
													$('.c' + id[el]).toggle();
													}
												}
